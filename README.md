# go-libs · latest published v0.3.0
...
> - 📜 **[Changelog & Release Record](CHANGELOG.md)** (latest published v0.3.0 release ledger, API errata, and evolution)
...
By having a single `github.com/umesh0492/go-libs@v0.3.0` import, you guarantee that all internal packages (`db`, `recovery`, `ratelimit`, `cache`, `workerpool`, `metrics`, `retry`, `telemetry`) are perfectly synced to the same tested release threshold across all your microservices. It prevents version drifting between interconnected middleware packages natively.
...
go get github.com/umesh0492/go-libs@v0.3.0
go mod tidy
...
| Package | Import suffix | Key exports & Description |
|---|---|---|
| `sliceutil` | `/sliceutil` | `Reduce`, `GroupBy`, `Chunk`, `Unique`, `Flatten`, `First` — generic slice algorithms beyond standard `slices` |
| `maputil` | `/maputil` | `Merge`, `Filter` — generic type-safe map operations beyond standard `maps` |
...
| `sliceutil` | Generic slice algorithms (Reduce, GroupBy, Chunk, Unique, Flatten, First) | **100.0%** |
